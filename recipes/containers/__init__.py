...
class SelfAwareContainer(object):
    ...
    in which case that object is returned. Use at your own discresion.
    """
    _skip_init = False
...
class TypeEnforcer(object):
    ...
    def check_type(self, obj, i):
        if not isinstance(obj, self.item_class):
            raise TypeError('Items contained in `%s` must derive from '
                            '`%s`. Item %i is of type %r.'
                            % (self.__class__.__name__,
                               self.item_class.__name__, i, type(obj)))
...
class MethodCaller(object):
    """
    This is adapted from `operator.methodcaller` with the `__call__` method
    altered to support chained attribute lookup like:
