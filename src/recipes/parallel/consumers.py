...
class BaseTask(LoggingMixin):
    ...
    def __str__(self):
        return f'{type(self).__name__}<{caller(self.func, self.args, self.kws)}>'
...
#         return Tasks([TriggeringTask(f, *pre_args, *args)
...
class AutoTriggerConsumer2(
        mp.Process, LoggingMixin):  # TODO: subclass Consumer above (when mature)
    """
        ...
        while True:
            ...
            try:
                ...
                # OR check if returned tuple contains a Task??
                    ...
                    self.logger.warning(
                        'no output queue to add returned {:s} to', returned)
                    # OR raise??
